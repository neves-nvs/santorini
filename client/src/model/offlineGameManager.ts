...
export default class OfflineGameManager implements GameManager {
  ...
  addPlayer(player: Player) {
    ...
    if (maxNumberPlayersReached)
      throw new Error("[addPlayer] Max Number of players reached.");

    let usernameAlreadyInUse = this.players.some(
      (p) => p.username == player.username,
    );
    if (usernameAlreadyInUse)
      throw new Error("[addPlayer] Repeated player name");
  ...
  getPlays(): Play[] {
    ...
    if (gameAlreadyFinished)
      throw new Error("[getMove] Game has already finished.");
  ...
  private buildGetHandler(): Play[] {
    ...
    // No builder selected
    if (!this.selectedBuilderPosition)
      throw Error("[getPlays:MOVE] no builder selected");
  ...
  private placePlayHandler(source: Position) {
    ...
    let allPlayersPlacedAllBuilders = this.builderPerPlayer.every(
