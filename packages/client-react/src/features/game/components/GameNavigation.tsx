  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: "0.75rem" }}>
        <div style={{ fontWeight: "bold", marginBottom: "0.25rem" }}>{appState.username || "Guest"}</div>
        <div style={{ fontSize: "0.8rem" }}>
          <span style={{ color: isConnected ? "#4CAF50" : "#f44336" }}>
            ● {isConnected ? "Connected" : "Disconnected"}
          </span>
        </div>
        {gameId && <div style={{ fontSize: "0.8rem", color: "#ccc", marginTop: "0.25rem" }}>Game: {gameId}</div>}
      </div>
